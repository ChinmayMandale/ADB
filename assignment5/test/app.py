...
def higestmag():
    ...
    if  request.method == "POST":  
        ...
        query_string = f"SELECT TOP {user} place, mag FROM earthquake \
                        ORDER BY mag DESC"   
...
#         user1 = request.form['user1']
#         user2 = request.form['user2']
#         n = request.form['user3']
#         name = [('Task','My task')]
#         m = int(n)
#         for i in range(int(user1), int(user2)):
#             query_string = f"select count(col2) as [number of occurences] \
#                             from data t\
#                             where t.col2 between {i} and {i+m}"   
#             cursor.execute(query_string)
...
#             query_string = f"select count(col2) as [number of occurences] \
#                             from data t\
#                             where t.col2 between {i} and {i+step}"   
#             cursor.execute(query_string)
#             temp = cursor.fetchone()
#             name.append((str(i)+'-'+str(i+step), temp[0]))
#             i+=step
...
def piechart():
    ...
    if  request.method == "POST":  
        ...
        query_string = f"SELECT MIN(col3) FROM data;"
        cursor.execute(query_string)
        user1 = cursor.fetchone()

        query_string = f"SELECT MAX(col3) FROM data;"
        cursor.execute(query_string)
        user2 = cursor.fetchone()

        n = int(request.form['n'])

        step = (user2[0]-user1[0])/n
        ...
        while i<user2[0]:
            ...
            query_string = f"select count(col3) as [number of occurences] \
                            from data t\
                            where col3 between {i} and {i+step}"   
            cursor.execute(query_string)
            temp = cursor.fetchone()
            name.append((str(i)+'-'+str(i+step), temp[0]))
            i+=step
